...
from sse_starlette.sse import EventSourceResponse
 ...
 # Use the hierarchical orchestrator script instead of the sequential runner
from pathlib import Path
ROOT = Path(__file__).parent.parent
HIERARCHICAL_SCRIPT = ROOT / "hierarchical_run_crew.py"
    ...
    if rpc_req.method == "recommendPR":
        ...
        if not repo_path:
            ...
            if isinstance(content, str) and content.startswith(tool_prefix):
                ...
                # Unescape any escaped quotes from the LLM output
                code = code.replace('\\"', '"')
                ...
                match = re.search(
                    ...
                    r'git_diff\.call\(query="(?P<call_path>[^"]+)"\)|'
                    r'git_diff\("(?P<simple_path>[^"]+)"\)',
                    code
                )
                if match:
                    raw = match.group("path") or match.group("call_path") or match.group("url") or match.group("simple_path")
                    ...
                    if raw.startswith("http"):
                        ...
                        try:
                            ...
                            cmd = [sys.executable, str(HIERARCHICAL_SCRIPT), tmp_dir]
                            ...
                            try:
                                ...
                                with Popen(cmd, stdout=PIPE, stderr=subprocess.STDOUT, text=True) as proc:
                                    ...
                                    output_files = glob.glob(os.path.join(tmp_dir, "outputs", "*.json"))
                                    # Load the final PR text from the most recently generated JSON
                                    latest_json = max(output_files, key=os.path.getmtime)
                                    print(f"Generated JSON file: {os.path.basename(latest_json)}")
                                    recommendation = json.load(open(latest_json)).get("pr_text", "")
                        ...
                        # Run the diff-to-PR CLI and capture its stdout
                        cmd = [sys.executable, str(HIERARCHICAL_SCRIPT), diff_arg]
                        ...
                        try:
                            ...
                            with Popen(cmd, stdout=PIPE, stderr=subprocess.STDOUT, text=True) as proc:
                                ...
                                pbar.close()
                                # collect JSON output files
                                output_files = glob.glob(os.path.join(diff_arg, "outputs", "*.json"))
                                # Load the final PR text from the most recently generated JSON
                                latest_json = max(output_files, key=os.path.getmtime)
                                print(f"Generated JSON file: {os.path.basename(latest_json)}")
                                recommendation = json.load(open(latest_json)).get("pr_text", "")
